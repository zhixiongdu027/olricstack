...
- **§8.6 / F13 (Phase 4 durable handoff) — IMPLEMENTED via synchronous partition drain (option A)**. `DurableHook` gained `DrainForHandoff(ctx, handoff)`. The fork's `fragment.Move` invokes it under `f.Lock()` before exporting, passing `DMap`, `PartitionID`, and `PartitionCount` plus resident hkeys as a compatibility hint. The hook routes to `MySQLStore.FlushHandoffPartition`, which scans the old owner's WAL for every committed record in that partition and flushes it to MySQL synchronously before deleting it from bbolt. This closes the resident-hkey gap surfaced by the 2026-05-18 cross-check: committed tombstones and evicted-but-dirty records may no longer exist in `frag.mem`, but they still belong to the migrating partition and must be drained. Drain failure aborts migration so the old owner can keep serving until the next retry. Regression tests in `internal/store/store_test.go` and `internal/stringkv/durable_hook_test.go` cover partition drain, resident-hkey fallback, prepared-skip, mysql-down abort, and empty-set no-op.
...
**Defense**:
- **Synchronous partition drain on handoff (Phase 4 option A, IMPLEMENTED)**: `fragment.Move` (`third_party/olric/internal/dmap/fragment.go`) now calls `DurableHook.DrainForHandoff` while still holding `f.Lock()`, passing the migrating `(DMap, PartitionID, PartitionCount)`. The hook (`internal/stringkv/durable_hook.go::DrainForHandoff`) prefers `MySQLStore.FlushHandoffPartition` (`internal/store/store.go`), which scans the WAL for all flushable records where `record.DMap = DMap` and `record.HKey % PartitionCount = PartitionID`, then flushes them to MySQL and removes them from bbolt. Resident hkeys remain only as a fallback path for non-partition-aware stores. This matters because committed tombstones and evicted-but-dirty records can be absent from `frag.mem` while still being part of the migrating partition.
- **Drain failure aborts migration**: if `FlushHandoffPartition` returns an error (MySQL outage, disk failure), `DrainForHandoff` propagates it, `fragment.Move` returns the error, and the cluster balancer leaves the fragment on N₁. The old owner remains write-serving until the next flush tick clears the WAL, at which point the next balancer cycle can retry. No silent data loss.
- **Prepared records left alone**: in-flight prepares (`WALStatePrepared`) are skipped by `FlushHandoffPartition` so their `BeforeX/AfterX` finalize protocol is unaffected. They cannot reach MySQL anyway (see §S2).
...
- **Implementation**:
  - `config.DurableHandoff{DMap, PartitionID, PartitionCount, HKeys}` carries the handoff descriptor. `HKeys` is a compatibility hint, not the authoritative WAL scope.
  - `stringkv.DurableHook.DrainForHandoff` prefers `LeaseGatedStore.FlushHandoffPartition` → `MySQLStore.FlushHandoffPartition`; it falls back to the older hkey-list `FlushHandoff` path only when the committer does not implement partition draining.
  - `FlushHandoffPartition` collects only `isFlushable` records (no Prepared, no records without a fence) in the migrating partition, runs them through the existing `flushEntries` + `versionedUpsertClause` path, then `deleteFlushed`.
  - On any error the WAL is left intact and the error propagates back up through the fork, aborting the migration.
- **Tests**: `TestMySQLStoreFlushHandoffDrainsCommittedRecords`, `TestMySQLStoreFlushHandoffPartitionDrainsNonResidentRecords`, `TestMySQLStoreFlushHandoffSkipsPreparedAndAbsentRecords`, `TestMySQLStoreFlushHandoffSurfacesFlushFailure`, `TestDurableHookDrainForHandoff{PrefersPartitionDrain,ForwardsToCommitStore,SurfacesFailure,NoOpOnEmpty}`.
